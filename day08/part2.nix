  ...
  findAntinodes = let
    findAntinodesInner = m: a: b:
      let
        ...
        res = (if isOk first then [ first ] else [ ])
          ++ (if isOk second then [ second ] else [ ])
          ++ (if isOk first || isOk second then
            findAntinodesInner (m + 1) a b
          else
            [ ]);
      in res;
  in findAntinodesInner 0;

  antinodes = builtins.foldl' (set: antennas:
    let
      allPoints = builtins.concatMap (a:
        builtins.concatMap (b: if a == b then [ ] else findAntinodes a b)
        antennas) antennas;
