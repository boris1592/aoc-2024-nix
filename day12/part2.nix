...
let
  ...
  isCorner = group: coord: delta:
    let
      ...
      getGroup = key: if groups ? ${key} then groups.${key} else -1;

      outerCorner = (getGroup sameRow != group && getGroup sameCol != group);
      innerCorner = (getGroup sameRow == group && getGroup sameCol == group
        && getGroup target != group);
    in outerCorner || innerCorner;

  sides = builtins.foldl' (total: c:
    let
      row = c.i;
      line = c.item;
    in builtins.foldl' (total: c:
      let
        ...
        corners = builtins.foldl'
          (sum: delta: if isCorner group curr delta then sum + 1 else sum) 0 [
            {
              row = -1;
              col = -1;
            }
  ...
  ans = builtins.foldl' (sum: group: sides.${group} * areas.${group} + sum) 0
