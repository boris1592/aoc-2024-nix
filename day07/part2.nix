let
  ...
  in
    ...
    then false
    else addCheck || mulCheck || concatCheck;
in
  input: let
    exprs = map (line: let
    ...
    okExprs = builtins.filter ({
      target,
      operands,
    }:
      check target (builtins.head operands) (builtins.tail operands))
    exprs;
