...
in
  input: let
    ...
    field = builtins.foldl' (field: l: let
      row = l.i;
      line = l.item;
    in
    ...
    traverse = field:
      builtins.genericClosure {
        ...
        in
          if !(isOk next) || (visited ? ${visKey})
          then []
          else if field ? ${builtins.toJSON next}
          then [(state' // {direction = util.mod (direction + 1) 4;})]
          else [(state' // {pos = next;})];
      };
    ...
    obstacles = builtins.filter (key: let
      ...
      hasLoop =
        ...
        ({
          ...
          visited ? ${stepKey pos direction})
        steps;
    in
      hasLoop) (builtins.attrNames stepSet);
